...
public:
    void onDataReceived(const uint8_t* data, size_t length) override {
        ...
        // std::cout << "[CanListener] Received " << length << " bytes" << std::endl;
    }
...
public:
    void onCommandReceived(uint32_t command, const uint8_t* data, size_t length) override {
        ...
        m_lastLength = length;
        // std::cout << "[CommandListener] Received cmd " << std::hex << command << std::dec << ", len " << length << std::endl;
    }
...
bool testCanfd(MockServer& server, TcpCanbusCommunication& client, MockCanListener& canListener) {
    ...
    return (canListener.m_receivedCount == 1 &&
            ...
            // We could check content too, but length is good indicator
}
...
bool testBadSize(MockServer& server, TcpCanbusCommunication& client, MockCanListener& canListener) {
    canListener.reset();
    // Send size > 1024
    server.sendV1Message("v1.00", CMD_CANBUS_DATA, nullptr, 2000);
...
int main() {
    ...
    printResult("Bad Data Size", testBadSize(server, client, canListener));
