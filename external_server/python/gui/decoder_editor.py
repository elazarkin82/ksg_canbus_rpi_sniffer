...
class DecoderEditorDialog(tk.Toplevel):
    ...
    PRESETS = {
        "(x[0]*256 + x[1])/4.0 (used for example for Engine RPM)": {"name": "Engine RPM", "start_byte": 0, "length": 2, "type": "formula", "scale": 1.0, "offset": 0.0, "formula": "(x[0]*256 + x[1])/4.0", "reverse_formula": "int(v * 4.0)"},
        "x[0] (used for example for Vehicle Speed)": {"name": "Vehicle Speed", "start_byte": 0, "length": 1, "type": "uint8", "scale": 1.0, "offset": 0.0, "formula": "x[0]", "reverse_formula": "int(v)"},
        "int.from_bytes(bytes([x[0], x[1]]), byteorder='big', signed=True) / 10.0 (used for example for Steering Angle)": {"name": "Steering Angle", "start_byte": 0, "length": 2, "type": "formula", "scale": 1.0, "offset": 0.0, "formula": "int.from_bytes(bytes([x[0], x[1]]), byteorder='big', signed=True) / 10.0", "reverse_formula": "int(v * 10.0)"},
        "(x[0]*256 + x[1])/4.0 (used for example for OBD2 Engine RPM)": {"name": "Engine RPM", "start_byte": 3, "length": 2, "type": "formula", "scale": 1.0, "offset": 0.0, "formula": "(x[0]*256 + x[1])/4.0", "reverse_formula": "int(v * 4.0)"},
        "x[0] (used for example for OBD2 Vehicle Speed)": {"name": "Vehicle Speed", "start_byte": 3, "length": 1, "type": "formula", "scale": 1.0, "offset": 0.0, "formula": "x[0]", "reverse_formula": "int(v)"},
        "x[0] - 40 (used for example for OBD2 Coolant Temp)": {"name": "Coolant Temp", "start_byte": 3, "length": 1, "type": "formula", "scale": 1.0, "offset": 0.0, "formula": "x[0] - 40", "reverse_formula": "int(v + 40)"},
        "x[0] * 100.0 / 255.0 (used for example for OBD2 Throttle Pos)": {"name": "Throttle Pos", "start_byte": 3, "length": 1, "type": "formula", "scale": 1.0, "offset": 0.0, "formula": "x[0] * 100.0 / 255.0", "reverse_formula": "int(v * 255.0 / 100.0)"},
        "x[0] * 100.0 / 255.0 (used for example for OBD2 Engine Load)": {"name": "Engine Load", "start_byte": 3, "length": 1, "type": "formula", "scale": 1.0, "offset": 0.0, "formula": "x[0] * 100.0 / 255.0", "reverse_formula": "int(v * 255.0 / 100.0)"},
        "x[0] * 100.0 / 255.0 (used for example for OBD2 Fuel Level)": {"name": "Fuel Level", "start_byte": 3, "length": 1, "type": "formula", "scale": 1.0, "offset": 0.0, "formula": "x[0] * 100.0 / 255.0", "reverse_formula": "int(v * 255.0 / 100.0)"},
        "(x[0]*256 + x[1])/100.0 (used for example for OBD2 MAF Air Flow)": {"name": "MAF Air Flow", "start_byte": 3, "length": 2, "type": "formula", "scale": 1.0, "offset": 0.0, "formula": "(x[0]*256 + x[1])/100.0", "reverse_formula": "int(v * 100.0)"},
        "x[0]/2.0 - 64.0 (used for example for OBD2 Timing Advance)": {"name": "Timing Advance", "start_byte": 3, "length": 1, "type": "formula", "scale": 1.0, "offset": 0.0, "formula": "x[0]/2.0 - 64.0", "reverse_formula": "int((v + 64.0) * 2.0)"},
        "x[0] (used for example for OBD2 Intake MAP)": {"name": "Intake MAP", "start_byte": 3, "length": 1, "type": "formula", "scale": 1.0, "offset": 0.0, "formula": "x[0]", "reverse_formula": "int(v)"},
        "x[0] * 3 (used for example for OBD2 Fuel Pressure)": {"name": "Fuel Pressure", "start_byte": 3, "length": 1, "type": "formula", "scale": 1.0, "offset": 0.0, "formula": "x[0] * 3", "reverse_formula": "int(v / 3.0)"},
        "x[0]*256+x[1] (used for example for OBD2 Run Time)": {"name": "Run Time", "start_byte": 3, "length": 2, "type": "formula", "scale": 1.0, "offset": 0.0, "formula": "x[0]*256+x[1]", "reverse_formula": "int(v)"},
        "x[0] * 100.0 / 255.0 (used for example for Generic 1 Byte Percent)": {"name": "Percent Value", "start_byte": 0, "length": 1, "type": "formula", "scale": 1.0, "offset": 0.0, "formula": "x[0] * 100.0 / 255.0", "reverse_formula": "int(v * 255.0 / 100.0)"},
    }

    def __init__(self, parent, can_id, current_signals, on_save_callback, pid=None, recorder=None, pid_index=0, profile_manager=None):
    ...
    def _setup_ui(self):
        ...
        # Map to Cockpit Button
        ttk.Button(controls_frame, text="Map to Cockpit Control...", command=self._map_to_control).pack(side=tk.RIGHT, padx=5)
    ...
    def _map_to_control(self):
        if not self.profile_manager:
            messagebox.showerror("Error", "Profile Manager not available.")
            return
            
        selected = self.tree_signals.selection()
        if not selected:
            messagebox.showwarning("Warning", "Please select a saved signal from the list to map.")
            return
            
        sig_index = self.tree_signals.index(selected[0])
        sig = self.signals[sig_index]
        
        # Create a mapping dialog
        map_dialog = tk.Toplevel(self)
        map_dialog.title("Map to Control")
        map_dialog.geometry("450x350")
        
        ttk.Label(map_dialog, text="Target Control:").grid(row=0, column=0, sticky="w", padx=10, pady=5)
        ctrl_var = tk.StringVar(value="steering")
        ttk.Combobox(map_dialog, textvariable=ctrl_var, values=["steering", "throttle", "brake"], state="readonly").grid(row=0, column=1, padx=10, pady=5)
        
        # Changed labels from "Raw" to "Physical/Decoded" based on the fix
        ttk.Label(map_dialog, text="Physical Min Value (e.g. -30.0):").grid(row=1, column=0, sticky="w", padx=10, pady=5)
        min_var = tk.StringVar(value="-30.0")
        ttk.Entry(map_dialog, textvariable=min_var).grid(row=1, column=1, padx=10, pady=5)
        
        ttk.Label(map_dialog, text="Physical Max Value (e.g. 30.0):").grid(row=2, column=0, sticky="w", padx=10, pady=5)
        max_var = tk.StringVar(value="30.0")
        ttk.Entry(map_dialog, textvariable=max_var).grid(row=2, column=1, padx=10, pady=5)
        
        ttk.Label(map_dialog, text="Reverse Formula (Encode):").grid(row=3, column=0, sticky="w", padx=10, pady=5)
        rev_var = tk.StringVar()
        
        # Auto-fill reverse formula if known from presets
        preset_name = self.preset_var.get()
        if preset_name in self.PRESETS and self.PRESETS[preset_name]["formula"] == sig.get("formula"):
            rev_var.set(self.PRESETS[preset_name].get("reverse_formula", "int(v)"))
        else:
            rev_var.set("int(v)")
            
        ttk.Entry(map_dialog, textvariable=rev_var, width=30).grid(row=3, column=1, padx=10, pady=5)
        ttk.Label(map_dialog, text="(Use 'v' as input. e.g., 'int(v * 4.0)')", foreground="gray").grid(row=4, column=1, sticky="w", padx=10)
        
        def save_mapping():
            try:
                min_v = float(min_var.get())
                max_v = float(max_var.get())
                
                # We need pid_val. If filtering is active, use that. Else use self.pid
                pid_val = self.pid
                if hasattr(self, 'pid_filter_var') and self.pid_filter_var.get() != "All":
                    pid_val = int(self.pid_filter_var.get(), 16)
                    
                self.profile_manager.set_control_mapping(
                    control_name=ctrl_var.get(),
                    can_id=self.can_id,
                    pid=pid_val,
                    signal_index=sig_index,
                    min_val=min_v,
                    max_val=max_v,
                    reverse_formula=rev_var.get(),
                    start_byte=sig.get("start_byte", 0),
                    length=sig.get("length", 1),
                    sig_type=sig.get("type", "uint8")
                )
                self.profile_manager.save()
                messagebox.showinfo("Success", f"Successfully mapped to {ctrl_var.get()}.", parent=map_dialog)
                map_dialog.destroy()
            except ValueError:
                messagebox.showerror("Error", "Min/Max values must be numbers.", parent=map_dialog)
                
        ttk.Button(map_dialog, text="Save Mapping", command=save_mapping).grid(row=5, column=0, columnspan=2, pady=20)
